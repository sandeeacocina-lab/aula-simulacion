...
function socialPage(kind,{ccc='00000000000',company='ARREA FICTICIA SL',count='2',badTotal=false,missingCcc=false,period='03/2026-03/2026',missingNaf=false,badDate=false,changedBase=false,missingAmount=false,deductions=false}={}){
 ...
 if(landscape){
  ...
  [['Contingencias comunes',base,'804,41'],['Equidad intergeneracional MEI',base,'25,58'],['Líquido cotizaciones generales','','829,99'],['IT Accidentes de trabajo',base,'22,74'],['IMS Accidentes de trabajo',base,'19,90'],...(deductions?[['Compensación por IT Accidente de trabajo','','100,00']]:[]),['Líquido accidentes de trabajo y enfermedades profesionales','',deductions?'- 57,36':'42,64'],['Otras cotizaciones',base,'244,23'],...(deductions?[['Bonificaciones','','1.000,00']]:[]),['Líquido otras cotizaciones','',deductions?'- 755,77':'244,23'],['Líquido de totales','',badTotal?'1.000,00':deductions?'16,86':'1.116,86']].forEach(([text,b,n],i)=>{t(17.88,290.76+i*17.61,text);if(b)t(453.1,290.76+i*17.61,b);t(533.62,290.76+i*17.61,n);});
 }
 return {width,height,items};
}
export function socialPdf(kind,options={}){
 const pages=(options.pages||[options]).map(o=>socialPage(kind,o));
 const objects=['<< /Type /Catalog /Pages 2 0 R >>',`<< /Type /Pages /Kids [${pages.map((_,i)=>`${4+i*2} 0 R`).join(' ')}] /Count ${pages.length} >>`,'<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>'];
 const literal=s=>s.replace(/[\\()]/g,'\\$&');
 for(const [index,{width,height,items}] of pages.entries()){
  const content=items.filter(i=>i.text).map(i=>`BT /F1 9.72 Tf 1 0 0 1 ${i.x} ${height-i.y} Tm (${literal(i.text)}) Tj ET`).join('\n');
  objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${width} ${height}] /Resources << /Font << /F1 3 0 R >> >> /Contents ${5+index*2} 0 R >>`,`<< /Length ${Buffer.byteLength(content,'latin1')} >>\nstream\n${content}\nendstream`);
 }
