...
async function prepareBackup(value:unknown){
 const b=value as Backup;if(!b||b.format!=='aula-simulacion'||b.version!==1||!b.tables||typeof b.tables!=='object'||Array.isArray(b.tables)||!b.storage||typeof b.storage!=='object'||Array.isArray(b.storage)||!Array.isArray(b.files)||Object.keys(b.tables).length!==tables.length)throw Error('Esta copia no es compatible con el aula.');
