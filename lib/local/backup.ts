...
async function prepareBackup(value:unknown){
 ...
 try{
  ...
  for(const name of tables){if((name.startsWith('documentation_')||name.startsWith('vf_'))&&!b.tables[name])continue;const t=b.tables[name],schema=db.exec(`PRAGMA table_info("${name}")`)[0].values,expected=schema.map(r=>String(r[1]));
   ...
   try{for(const source of t.rows){if(!Array.isArray(source)||source.length!==t.columns.length)throw Error('La copia contiene datos no válidos.');
     ...
     for(const key of ['data','warnings','attachments','receipt','receipt_details','payload']){const i=expected.indexOf(key);if(i>=0&&row[i]!==null)JSON.parse(String(row[i]));}
