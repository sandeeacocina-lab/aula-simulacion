...
async function plan(scope:Scope,id?:string){
 ...
 if(has('verifactu')){if(id)throw new PracticeError('Reinicia el módulo completo para conservar la coherencia del historial.');const invoices=await rows('vf_invoices');counts['Facturas y borradores']=invoices.length;counts['Registros VERI*FACTU']=(await rows('vf_records')).length;files.push(...invoices.map(x=>x.source_key).filter(Boolean));statements.push(db().prepare("DELETE FROM vf_registry WHERE company_id='demo'"),db().prepare("DELETE FROM vf_records WHERE company_id='demo'"),db().prepare("DELETE FROM vf_invoices WHERE company_id='demo'"));}
