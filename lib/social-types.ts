...
export type SocialDocument={kind:SocialKind;filename:string;header:SocialHeader;lines:SocialLine[];amounts:SocialAmount[];total:number|null;pages:number;warnings:string[]};
export type SocialDuplicate={batchId:string;reference:string;kind:SocialKind;filename:string};
export type SocialPreview={documents:SocialDocument[];fingerprint:string;warnings:string[];duplicates:SocialDuplicate[]};
