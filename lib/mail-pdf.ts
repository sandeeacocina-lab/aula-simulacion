...
export function mailPDF(m:MailMessage){
 ...
 const header=()=>{const scale=Math.min(120/mailLogo.width,45/mailLogo.height),w=mailLogo.width*scale,h=mailLogo.height*scale;stream+=`q ${w} 0 0 ${h} 44 ${842-40-h} cm /Logo Do Q\n`;text('CORREO',455,55,12,true,pink);text(m.homeFolder==='sent'?'Enviado':m.homeFolder==='drafts'?'Borrador':'Recibido',455,73,9,false,muted);rule(89);y=113;};
 ...
 y+=12;ensure(35);rule(y);y+=25;paragraph(m.body||'Sin texto',10.5);y+=22;
 if(m.source==='mail'){
 ...
 const logo=add(objStream(atob(mailLogo.data),`/Type /XObject /Subtype /Image /Width ${mailLogo.width} /Height ${mailLogo.height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode`));
 const pageIds=pages.map(content=>{const ref=add(objStream(content));return add(`<< /Type /Page /Parent ${root} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontIds[0]} 0 R /F2 ${fontIds[1]} 0 R >> /XObject << /Logo ${logo} 0 R >> >> /Contents ${ref} 0 R >>`);});
