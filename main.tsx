...
import {initializeWorkspaces,getWorkspaceId,selectWorkspace} from './lib/local/workspaces';
...
import './styles/verifactu.css';
const Verifactu=lazy(()=>import('./components/verifactu/workspace'));
...
const SystemRed=lazy(()=>import('./components/social/system-red'));
...
const routes:Record<string,ReactNode>={'/':<Home/>,'/servicios':<CompanyHome/>,'/practicas':<Settings/>,'/servicios/agencia-tributaria/verifactu':<Verifactu/>,'/servicios/documentacion':<Documentation/>,'/servicios/banco':<Bank/>,'/servicios/banco/justificante':<BankReceipt/>,'/servicios/banco/financiacion':<FinanceReceipt/>,'/servicios/correo':<Mail/>,'/servicios/agencia-tributaria':<div className="tax-app"><Tax/></div>,'/servicios/sepe':<Sepe/>,'/servicios/sepe/justificante':<SepeReceipt/>,'/servicios/seguridad-social':<Social/>,'/servicios/seguridad-social/sistema-red':<SystemRed/>,'/servicios/seguridad-social/cotizacion':<Contributions/>,'/servicios/seguridad-social/justificante':<SocialReceipt/>,'/servicios/seguridad-social/registros':<RegistryHistory/>,'/servicios/seguridad-social/empresas':<Registry kind="company"/>,'/servicios/seguridad-social/afiliacion':<Registry kind="person"/>,'/servicios/seguridad-social/altas':<Registry kind="employment"/>};
...
function DownloadPage({url}:{url:string}){
 ...
 return <main className="aula-main aula-file-preview"><h1>{error?'No se ha podido abrir':ready?'Archivo preparado':'Preparando el archivo…'}</h1><p role="status">{error||(ready&&!preview?'La descarga se ha iniciado. Puedes cerrar esta pestaña.':'')}</p>{preview&&<iframe title="Vista previa del adjunto" src={preview}/>}<a href="#/servicios">Volver a los servicios</a></main>;
}
function App(){const [ready,setReady]=useState(false),[initError,setInitError]=useState('');useEffect(()=>{let active=true;initializeWorkspaces(new URLSearchParams(window.location.hash.split('?')[1]||'').get('empresa')).then(()=>{if(active)setReady(true);}).catch(e=>{if(active)setInitError(e.message);});return()=>{active=false;};},[]);const [hash,setHash]=useState(window.location.hash||'#/');useEffect(()=>{const change=()=>{if(window.location.hash.startsWith('#/')){const empresa=new URLSearchParams(window.location.hash.split('?')[1]||'').get('empresa');if(empresa&&empresa!==getWorkspaceId()){try{selectWorkspace(empresa);window.location.reload();}catch(e){setReady(false);setInitError((e as Error).message);}return;}setHash(window.location.hash);window.scrollTo(0,0);}};const skip=(e:MouseEvent)=>{const a=(e.target as Element).closest?.('a'),href=a?.getAttribute('href');if(href?.startsWith('#')&&!href.startsWith('#/')){const target=document.getElementById(href.slice(1));if(target){e.preventDefault();target.scrollIntoView();target.setAttribute('tabindex','-1');target.focus();}}};window.addEventListener('hashchange',change);window.addEventListener('popstate',change);document.addEventListener('click',skip);return()=>{window.removeEventListener('hashchange',change);window.removeEventListener('popstate',change);document.removeEventListener('click',skip);};},[]);const path=hash.slice(1).split('?')[0]||'/';if(!ready)return <main className="aula-main company-loading"><p role={initError?'alert':'status'}>{initError||'Preparando las empresas…'}</p>{initError&&<a href="./">Elegir empresa</a>}</main>;return <Boundary key={path==='/servicios/sepe'?path:hash}><Suspense fallback={<p className="company-loading" role="status">Abriendo el servicio…</p>}>{path==='/descargar'?<DownloadPage url={new URLSearchParams(hash.split('?')[1]).get('url')||''}/>:routes[path]||<main className="aula-main"><h1>No encontramos esa página.</h1><a href="#/">Volver al inicio</a></main>}</Suspense></Boundary>;}
export {App};
